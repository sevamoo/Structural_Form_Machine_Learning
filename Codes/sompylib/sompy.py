...
#     		plt.scatter(X, msz[0]-1-Y, s=area, alpha=0.9,c='None',marker='o',cmap='jet',linewidths=3, edgecolor = 'r')    		
    ...
    def view_U_matrix(self,distance2=1,row_normalized='No',show_data='No',contooor='Yes',blob = 'No',save='No',save_dir = ''):
    	import scipy
...
#          	weights = np.exp(weights)/S_
    ...
    def para_bmu_find(self, x, y, njb = 1):
        ...
        (self, x[i*dlen // njb:min((i+1)*dlen // njb, dlen)],y, Y2) for i in range(njb))
        t1 = time()
